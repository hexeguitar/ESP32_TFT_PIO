...
//---------------------------------------------------------------------------------------------------------------------
//CYD_Audio::CYD_Audio(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_DAC_CHANNEL_BOTH_EN */, uint8_t i2sPort) 
void CYD_Audio::begin(bool internalDAC /* = false */, uint8_t channelEnabled /* = I2S_DAC_CHANNEL_BOTH_EN */, uint8_t i2sPort) 
{
    ...
    if (internalDAC)  {
			...
			fillDACbuf(0x80008000);
			rms.setBias(0x8000); // remove DC bias in rms calculation
        #endif
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setBufsize(int rambuf_sz, int psrambuf_sz) {
    if(InBuff.isInitialized()) {
        log_e("CYD_Audio::setBufsize must not be called after audio is initialized");
        return;
    }
    InBuff.setBufsize(rambuf_sz, psrambuf_sz);
};

void CYD_Audio::initInBuff() {
    if(!InBuff.isInitialized()) {
...
//---------------------------------------------------------------------------------------------------------------------
esp_err_t CYD_Audio::I2Sstart(uint8_t i2s_num) {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setConnectionTimeout(uint16_t timeout_ms, uint16_t timeout_ms_ssl){
    if(timeout_ms)     m_timeout_ms     = timeout_ms;
    if(timeout_ms_ssl) m_timeout_ms_ssl = timeout_ms_ssl;
}

//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::connecttohost(const char* host, const char* user, const char* pwd) {
...
//    } fix in V2.0.8
    ...
    if(res){
		...
		m_fader = 0;						// start with 0 volume
		m_play_status = FADE_IN;				
		if(m_f_internalDAC) m_dacBias = 0;	// start with 0 bias and ramp it up
    }
    else{
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::httpPrint(const char* host) {
    // user and pwd for authentification only, can be empty
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setFileLoop(bool input){
    if(m_codec == CODEC_M4A) return 0;
    m_f_loop = input;
    return input;
}
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::UTF8toASCII(char* str){
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::connecttoSD(const char* path, int32_t resumeFilePos) {
    return connecttoFS(SD, path, resumeFilePos);
}
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::connecttoFS(fs::FS &fs, const char* path, int32_t resumeFilePos) {
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::connecttospeech(const char* speech, const char* lang){
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::urlencode(char* buff, uint16_t buffLen, bool spacesOnly) {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::showID3Tag(const char* tag, const char* value){
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::unicode2utf8(char* buff, uint32_t len){
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::latinToUTF8(char* buff, size_t bufflen){
...
//---------------------------------------------------------------------------------------------------------------------
size_t CYD_Audio::readAudioHeader(uint32_t bytes){
...
//---------------------------------------------------------------------------------------------------------------------
int CYD_Audio::read_WAV_Header(uint8_t* data, size_t len) {
...
//---------------------------------------------------------------------------------------------------------------------
int CYD_Audio::read_FLAC_Header(uint8_t *data, size_t len) {
    static size_t headerSize;
...
//---------------------------------------------------------------------------------------------------------------------
int CYD_Audio::read_ID3_Header(uint8_t *data, size_t len) {
...
//---------------------------------------------------------------------------------------------------------------------
int CYD_Audio::read_M4A_Header(uint8_t *data, size_t len) {
/*
       ftyp
...
//---------------------------------------------------------------------------------------------------------------------
size_t CYD_Audio::process_m3u8_ID3_Header(uint8_t* packet){
    uint8_t         ID3version;
    size_t          id3Size;
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::stopSong() {
    uint32_t pos = 0;
	
    if(m_f_running) 
	{
		if(m_f_internalDAC) { genFadeOut(); }
		else 				{ fillDACbuf(0x00); }
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::readPlayListData() {
...
//----------------------------------------------------------------------------------------------------------------------
const char* CYD_Audio::parsePlaylist_M3U(){
...
//----------------------------------------------------------------------------------------------------------------------
const char* CYD_Audio::parsePlaylist_PLS(){
    uint8_t lines = m_playlistContent.size();
    int pos = 0;
    char* host = nullptr;
...
//----------------------------------------------------------------------------------------------------------------------
const char* CYD_Audio::parsePlaylist_ASX(){                             // Advanced Stream Redirector
    uint8_t lines = m_playlistContent.size();
    bool f_entry = false;
    int pos = 0;
    char* host = nullptr;
...
//----------------------------------------------------------------------------------------------------------------------
const char* CYD_Audio::parsePlaylist_M3U8(){
    uint8_t lines = m_playlistContent.size();
    bool f_begin = false;
    uint8_t occurence = 0;
    if(lines){
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::STfromEXTINF(char* str){
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::processLocalFile() {
...
//----------------------------------------------------------------------------------------------------------------------
void CYD_Audio::processWebStream() {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::processWebStreamTS() {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::processWebStreamHLS() {
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::parseHttpResponseHeader() { // this is the response to a GET / request
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio:: initializeDecoder(){
    uint32_t gfH = 0;
    uint32_t hWM = 0;
    switch(m_codec){
        case CODEC_MP3:
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::parseContentType(char* ct) {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::showstreamtitle(const char* ml) {
    // example for ml:
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::showCodecParams(){
...
//---------------------------------------------------------------------------------------------------------------------
int CYD_Audio::findNextSync(uint8_t* data, size_t len){
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setDecoderItems(){
    if(m_codec == CODEC_MP3){
...
//---------------------------------------------------------------------------------------------------------------------
int CYD_Audio::sendBytes(uint8_t* data, size_t len) {
    int bytesLeft;
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::compute_audioCurrentTime(int bd) {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::printDecodeError(int r) {
    const char *e;
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN, int8_t MCK) {
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::getFileSize() {
    if(!audiofile) return 0;
    return audiofile.size();
}
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::getFilePos() {
    if(!audiofile) return 0;
    return audiofile.position();
}
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::getAudioDataStartPos() {
    if(!audiofile) return 0;
    return m_audioDataStart;
}
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::getAudioFileDuration() {
...
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::getAudioCurrentTime() {  // return current time in seconds
    return (uint32_t) m_audioCurrentTime;
}
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setAudioPlayPosition(uint16_t sec){
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setVolumeSteps(uint8_t steps) {
    m_vol_steps = steps;
    if (steps < 1)  m_vol_steps = 64; /* avoid div-by-zero :-) */
}
//---------------------------------------------------------------------------------------------------------------------
uint8_t CYD_Audio::maxVolume() {
        return m_vol_steps;
};
//---------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::getTotalPlayingTime() {
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setTimeOffset(int sec){
    if(m_codec == CODEC_OPUS) return false; // not impl. yet
    if(m_codec == CODEC_VORBIS) return false; // not impl. yet
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setFilePos(uint32_t pos) {
    if(m_codec == CODEC_OPUS) return false;   // not impl. yet
    if(m_codec == CODEC_VORBIS) return false; // not impl. yet
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::audioFileSeek(const float speed) {
    // 0.5 is half speed
    // 1.0 is normal speed
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setSampleRate(uint32_t sampRate) {
...
uint32_t CYD_Audio::getSampleRate(){
    return m_sampleRate;
}
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setBitsPerSample(int bits) {
    if((bits != 16) && (bits != 8)) return false;
    m_bitsPerSample = bits;
    return true;
}
uint8_t CYD_Audio::getBitsPerSample(){
    return m_bitsPerSample;
}
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setChannels(int ch) {
    if((ch < 1) || (ch > 2)) return false;
    m_channels = ch;
    return true;
}
uint8_t CYD_Audio::getChannels(){
...
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::setBitrate(int br){
    m_bitRate = br;
    if(br)return true;
    return false;
}
uint32_t CYD_Audio::getBitRate(bool avg){
    if (avg)
        return m_avr_bitrate;
    return m_bitRate;
}
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setI2SCommFMT_LSB(bool commFMT) {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::computeVUlevel(int16_t sample[2]){
...
//---------------------------------------------------------------------------------------------------------------------
uint16_t CYD_Audio::getVUlevel() {
    // avg 0 ... 127
    if(!m_f_running) return 0;
    return (m_vuLeft << 8) + m_vuRight;
}
//---------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::playSample(int16_t sample[2]) {
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setTone(int8_t gainLowPass, int8_t gainBandPass, int8_t gainHighPass){
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::forceMono(bool m) { // #100 mono option
    m_f_forceMono = m; // false stereo, true mono
}
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::setBalance(int8_t bal){ // bal -16...16
    if(bal < -16) bal = -16;
    if(bal >  16) bal =  16;
    m_balance = bal;
...
//---------------------------------------------------------------------------------------------------------------------
// replaced by new version in CYD_audioCustom.cpp

// void CYD_Audio::setVolume(uint8_t vol, uint8_t curve) { // curve 0: default, curve 1: flat at the beginning
...
// }
//---------------------------------------------------------------------------------------------------------------------
uint8_t CYD_Audio::getVolume() {
    return m_vol;
}
//---------------------------------------------------------------------------------------------------------------------
uint8_t CYD_Audio::getI2sPort() {
    return m_i2s_num;
}
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::computeLimit(){    // is calculated when the volume or balance changes
...
int32_t CYD_Audio::Gain(int16_t s[2]) {
    int32_t v[2];
...
//---------------------------------------------------------------------------------------------------------------------
void CYD_Audio::IIR_calculateCoefficients(int8_t G0, int8_t G1, int8_t G2){  // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* CYD_Audio::IIR_filterChain0(int16_t iir_in[2], bool clear){  // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* CYD_Audio::IIR_filterChain1(int16_t iir_in[2], bool clear){  // Infinite Impulse Response (IIR) filters
...
//---------------------------------------------------------------------------------------------------------------------
int16_t* CYD_Audio::IIR_filterChain2(int16_t iir_in[2], bool clear){  // Infinite Impulse Response (IIR) filters
...
//----------------------------------------------------------------------------------------------------------------------
bool CYD_Audio::ts_parsePacket(uint8_t* packet, uint8_t* packetStart, uint8_t* packetLength) {
...
//----------------------------------------------------------------------------------------------------------------------
uint16_t CYD_Audio::readMetadata(uint16_t maxBytes, bool first) {
...
//----------------------------------------------------------------------------------------------------------------------
size_t CYD_Audio::chunkedDataTransfer(uint8_t* bytes){
...
//----------------------------------------------------------------------------------------------------------------------
boolean CYD_Audio::streamDetection(uint32_t bytesAvail){
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::m4a_correctResumeFilePos(uint32_t resumeFilePos){
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::flac_correctResumeFilePos(uint32_t resumeFilePos){
...
//----------------------------------------------------------------------------------------------------------------------
uint32_t CYD_Audio::mp3_correctResumeFilePos(uint32_t resumeFilePos){
...
//----------------------------------------------------------------------------------------------------------------------
uint8_t CYD_Audio::determineOggCodec(uint8_t* data, uint16_t len){
